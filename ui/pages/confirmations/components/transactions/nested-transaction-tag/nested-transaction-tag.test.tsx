...
import { BatchTransactionParams } from '@serbaapp/transaction-controller';
...
function render({ nestedTransactions }: { nestedTransactions?: BatchTransactionParams[] }) {
...
describe('NestedTransactionTag', () => {
  const useNestedLabelsMock = jest.mocked(useNestedTransactionLabels);

  beforeEach(() => {
    jest.resetAllMocks();
    useNestedLabelsMock.mockReturnValue([FUNCTION_NAME_MOCK, FUNCTION_NAME_MOCK]);
  });

  it('renders tag label with the right number of transactions', () => {
    useNestedLabelsMock.mockReturnValue([FUNCTION_NAME_MOCK]);

    const { getByText } = render({
      nestedTransactions: [BATCH_TRANSACTION_PARAMS_MOCK, BATCH_TRANSACTION_PARAMS_MOCK],
    });
  ...
  it('renders correct tooltip content', () => {
    ...
    expect(container.querySelector('[data-original-title]')?.getAttribute('data-original-title'))
      .toBe('This transaction includes: TestFunction, TestFunction.');
  });
